 ...
 */
function getIdentityBackgroundColor(email) {
...
export default function Gravatar({ defaultImage = 'initials', displayName, email, size = 48, className, useHovercard = true, }) {
    ...
    const bgColorParam = defaultImage === 'initials' ? `&bg_color=${getIdentityBackgroundColor(email)}` : '';
    return (_jsx("img", { ref: profileImageRef, className: clsx('jetpack-components-gravatar', className), alt: displayName || '', src: `https://secure.gravatar.com/avatar/${hashedEmail}?d=${defaultImage}${hovercardName}${bgColorParam}`, width: size, height: size, loading: "lazy" }));
}
