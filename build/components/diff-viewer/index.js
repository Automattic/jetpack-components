...
export const DiffViewer = ({ diff }) => (_jsx("div", { className: styles['diff-viewer'], children: parsePatch(diff).map((file, fileIndex) => (_jsxs(Fragment, { children: [
            _jsx("div", { className: styles['diff-viewer__filename'], children: filename(file) }, `file-${fileIndex}`), _jsxs("div", { className: styles['diff-viewer__file'], children: [
                    _jsx("div", { className: styles['diff-viewer__line-numbers'], children: file.hunks.map((hunk, hunkIndex) => {
                            let lineOffset = 0;
